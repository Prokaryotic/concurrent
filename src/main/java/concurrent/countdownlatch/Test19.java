 ...
 *
 * 1.给lists加上volatile后t2能够接收到通知，但是，t2线程的死循环很浪费CPU ，如果不用死循环，应该怎么做
 *
 * 2.这使用了wait和notify做到，wait会释放锁，而notify不会释放锁
 *
 ...
 * 被唤醒后继续执行wait后面的代码
 *
 *3.因为notify是不会释放锁的所以需要在T1线程里加上一个wait去释放锁 在T2里加一个notify去唤醒T1让T1继续执行
 *
            ...
            synchronized (lock){
                ...
                for (int i = 0; i < 20; i++) {
                    ...
                    if (5 == t.size()){
                        ...
                        try {
                            lock.wait();//TODO  注意这里这里让T1释放锁 让T2获取锁并开始执行接下来的代码 T2结束后通过notify唤醒T1
                        } catch (InterruptedException e) {
                            e.printStackTrace();
                        }
                    }
